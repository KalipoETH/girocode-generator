...
export const metadata: Metadata = {
  title: "Qu'est-ce qu'un GiroCode? – Expliqué simplement + créer gratuitement",
  description:
    'GiroCode = code QR SEPA pour virements. Ce que c\'est, comment ça fonctionne et comment en créer un gratuitement en quelques secondes. PDF de facture inclus.',
  alternates: {
    canonical: `${SITE_URL}/fr/wissen/girocode`,
    languages: {
...
export default function WissenGirocodePageFr() {
  return (
    <KnowledgeLayout
      ...
      lead="Faire payer des factures sans faute de frappe, collecter des dons d'un simple scan, ne plus jamais dicter un IBAN – c'est à cela que sert le GiroCode. Ce guide explique ce qui se cache derrière, qui l'utilise et comment créer le vôtre en quelques secondes."
      relatedArticles={[
      ...
      statsData={[
        { value: '2018', label: "Année d'introduction en Allemagne" },
        { value: '36', label: 'Pays SEPA supportent la norme' },
        { value: '10 sec.', label: 'Temps de scan moyen' },
        { value: 'EPC069-12', label: 'Nom du document standard officiel' },
      ]}
    >
      ...
      <section aria-labelledby="comparaison-fr" className="mt-10">
        ...
        <div className="not-prose mb-4 rounded-lg border-l-4 border-[#22c55e] bg-[#133018]/40 px-4 py-3 text-sm leading-relaxed text-slate-300">
          <strong className="text-slate-100">Résumé comparatif :</strong> Le GiroCode est gratuit et conforme RGPD,
          tandis que PayPal prélève 1,2–3,4&nbsp;% de frais et est une entreprise américaine soumise à des règles de confidentialité différentes.
          Le GiroCode fonctionne directement via votre banque – sans inscription à un tiers.
        </div>
      ...
      {/* ── Comparaisons ─── */}
      <section aria-labelledby="comparaisons" className="mt-10">
        <h2 id="comparaisons">GiroCode en comparaison</h2>
        <p>Comment le GiroCode se compare-t-il aux autres méthodes de paiement ?</p>
        <div className="not-prose my-4 grid gap-3 sm:grid-cols-3">
          {[
            { href: '/fr/girocode-vs-paypal', title: 'GiroCode vs. PayPal', desc: 'Coûts, confidentialité et cas d\'utilisation comparés' },
            { href: '/fr/girocode-vs-bankueberweisung', title: 'GiroCode vs. virement manuel', desc: 'Temps, taux d\'erreur et facilité d\'utilisation' },
            { href: '/fr/girocode-vs-lastschrift', title: 'GiroCode vs. prélèvement SEPA', desc: 'Quelle méthode est la meilleure selon le cas' },
          ].map((card) => (
            <a
              key={card.href}
              href={card.href}
            ...
            >
              ...
              <span className="mt-3 text-xs text-[#22c55e]">Voir la comparaison →</span>
            </a>
          ))}
        </div>
      </section>
